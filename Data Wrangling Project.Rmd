...
Work with Google geocoding and ggmap packet to obtain the CAP
```{r}
...
data_clean <- data_clean %>% mutate(dep_to_bs = as.Date( x = dep_to_bs, format = "%d / %m / %y"),
                                    ...
                                    delivery_day = as.Date(x = delivery_day, format = "%d / %m / %y"))
```

_ Trasform dates in a better format 
```{r}
data_clean <- data_clean %>% mutate(dep_to_bs = format( x = dep_to_bs, format = "%d/%m/%y"),
                                    arr_to_bs = format(x = arr_to_bs, format = "%d/%m/%y"),
                                    delivery_day = format(x = delivery_day, format = "%d/%m/%y" ))
```
...
```{r}
data_clean <- data_clean %>% mutate(weekday_deliv = format(delivery_day, "%a %d"))  # FIX ISSUE: DAYS EXPRESSED IN ITALIAN
```
...
_ Convert character string to Hour regarding the variable "pickup_time"
```{r}
# HOW TO DO IT? as.POSITXct looks not totally helpful
```
